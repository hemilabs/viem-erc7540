...
import { claimableRedeemRequest } from "../../src/public/claimableRedeemRequest";
...
describe("claimableRedeemRequest", function () {
  it("should throw an error if the address is not valid", async function () {
    ...
    await expect(claimableRedeemRequest(client, parameters)).rejects.toThrow(
      "Invalid address",
    );
  });
  ...
  it("should call readContract if all parameters are valid", async function () {
    await claimableRedeemRequest(client, validParameters);
  ...
  it("should handle empty parameters gracefully", async function () {
    // @ts-expect-error - Testing invalid input
    await expect(claimableRedeemRequest(client, {})).rejects.toThrow(
      "Invalid address",
    );
  });
  ...
  it("should handle no parameters gracefully", async function () {
    // @ts-expect-error - Testing invalid input
    await expect(claimableRedeemRequest(client, undefined)).rejects.toThrow(
      "Invalid address",
    );
  });
});
